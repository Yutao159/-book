...
os.chdir(r"C:\Users\y1598\IdeaProjects\book-recommendation-system")

# Fix SSL certificate verification issue
os.environ['CURL_CA_BUNDLE'] = ''
os.environ['REQUESTS_CA_BUNDLE'] = ''

# Configure huggingface hub to use a custom session with SSL verification disabled
session = requests.Session()
session.verify = False
configure_http_backend(backend_factory=lambda: session)

# Create features folder if not exists
features_dir = r"C:\Users\y1598\IdeaProjects\book-recommendation-system\data\features"
if not os.path.exists(features_dir):
    os.makedirs(features_dir)
    print(f"Created folder: {features_dir}")

# Load cleaned data (absolute path + encoding)
data_dir = r"C:\Users\y1598\IdeaProjects\book-recommendation-system\data\processed"
books = pd.read_csv(os.path.join(data_dir, "books.csv"), encoding='latin-1')
users = pd.read_csv(os.path.join(data_dir, "users.csv"), encoding='latin-1')
merged_data = pd.read_csv(os.path.join(data_dir, "merged_data.csv"), encoding='latin-1')
...
if __name__ == "__main__":
    # Configuration: Set to True to skip BERT title embedding (avoids model download)
    SKIP_BERT_EMBEDDING = True  # Default to True to avoid network dependency issues
    
    try:
        # 1. Basic feature encoding
        print("Starting basic feature encoding...")
        books_encoded, users_encoded = encode_basic_features()
        
        # 2. Title embedding (time-consuming)
        if not SKIP_BERT_EMBEDDING:
            print("\nStarting BERT title embedding... (This may take time)")
            books_with_emb = generate_title_embedding(books_encoded)
        else:
            print("\nBERT title embedding skipped as configured")
            books_with_emb = books_encoded
        
        # 3. Interaction features
        print("\nStarting interaction features generation...")
        ...
        if "SSL" in str(e) or "certificate" in str(e).lower():
            print("\n💡 Suggestion: Set SKIP_BERT_EMBEDDING = True at line 25 to skip BERT embedding and avoid SSL issues")
        raise