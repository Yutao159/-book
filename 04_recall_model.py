...
# Prefer tensorflow.keras but fall back to standalone keras if tensorflow.keras is not available
try:
    ...
    from keras.layers import Input, Dense, Embedding, Concatenate, Flatten, Dot
    from keras.optimizers import Adam
import os
import json  # Add json for standardized embedding saving

# Force set working directory
os.chdir("C:\\Users\\y1598\\IdeaProjects\\book-recommendation-system")

# Create features folder if not exists
features_dir = "C:\\Users\\y1598\\IdeaProjects\\book-recommendation-system\\data\\features\\"
if not os.path.exists(features_dir):
    os.makedirs(features_dir)

# Load feature data (absolute path)
book_basic = pd.read_csv(f"{features_dir}book_basic_features.csv", encoding='utf-8-sig')
book_interact = pd.read_csv(f"{features_dir}book_interact_features.csv", encoding='utf-8-sig')
user_basic = pd.read_csv(f"{features_dir}user_basic_features.csv", encoding='utf-8-sig')
user_interact = pd.read_csv(f"{features_dir}user_interact_features.csv", encoding='utf-8-sig')
merged_data = pd.read_csv(r"C:\Users\y1598\IdeaProjects\book-recommendation-system\data\processed\merged_data.csv", encoding='utf-8-sig')

# Clean column names (remove any BOM characters)
book_basic.columns = book_basic.columns.str.replace('ï»¿', '', regex=False)
book_interact.columns = book_interact.columns.str.replace('ï»¿', '', regex=False)
user_basic.columns = user_basic.columns.str.replace('ï»¿', '', regex=False)
user_interact.columns = user_interact.columns.str.replace('ï»¿', '', regex=False)
merged_data.columns = merged_data.columns.str.replace('ï»¿', '', regex=False)
